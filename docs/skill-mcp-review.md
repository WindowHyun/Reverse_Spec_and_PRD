...
### 13.9 `<`의 진짜 모호성 — 비교연산자 vs JSX 태그 (2026-09-15)

§13.8 상태기계 직후 4번째로 같은 함수에서 또 발견됐다 — 이번엔 얕은 실수가
아니라 **문법적으로 진짜 모호한** 사례였다: `a <b && c > d ? <One/> : <Two/>`
에서 `<b`(공백 없이 식별자가 바로 붙음)는 태그 시작(`<Tag`)과 구문상 전혀
구분이 안 된다 — TypeScript 컴파일러도 겪는 유명한 모호성(`.tsx`에서 제네릭
`<T>`와 JSX가 충돌하는 것과 같은 종류)이다. §13.8의 상태기계는 이를 태그로
오인해 그 뒤의 `>`(비교연산자)를 태그 종료로 잘못 소비했고, 결과적으로 실제
구조적 `}`를 지나쳐 파일의 라우트가 전부 사라졌다.

실제 컴파일러/트랜스파일러가 쓰는 것과 같은 표준 해소 규칙을 적용했다: `<`
직전(공백 제외)이 식별자 문자나 닫는 괄호/대괄호/중괄호면 "방금 값이 끝난
자리"이므로 그 `<`는 십중팔구 연산자, 반대로 `(`/`{`/`,`/`:`/`?`/다른 연산자/
시작 위치라면 "표현식이 새로 시작하는 자리"라 태그 시작으로 판정
(`_looks_like_operator_lt`). **자체 재검토에서 2건 추가 발견 후 수정**:
(1) 처음엔 `search(src, 0, idx)` 정규식으로 구현했는데, `<`가 잦은 입력에서
호출마다 idx에 비례해 훑을 수 있어 이차식 비용을 재도입할 뻔했다 — 공백만
건너뛰는 짧은 역방향 스캔으로 교체해 호출당 비용이 사실상 상수가 되도록 함.
(2) 이 판정을 맥락 구분 없이 걸었다가, `<div>Don't stop</div>`의 `</div>`가
"stop"(식별자로 끝나는 텍스트) 뒤에 온다는 이유로 태그가 아니라고 오판해
§13.7이 재발하는 회귀를 자체 테스트에서 잡았다 — 이 모호성은 "code" 맥락
(표현식 안)에서만 존재하고 "text" 맥락(JSX 자식)에서는 `<` 뒤에 문자/`/`가
오면 무조건 태그이므로, 판정을 `mode == "code"`일 때만 적용하도록 좁혀 해결.

재검증: 이번 재현 케이스 + §13.6~13.8 전체 재현 케이스(개별 및 복합) +
자체 발견한 회귀 2건에 대한 대상 테스트(텍스트가 식별자로 끝나고 닫는 태그가
오는 케이스, 공백 패딩된 `<` 대량 반복 스트레스 케이스) + ReDoS 재현 케이스
여전히 빠름(~0.34~0.38초) + mock-shop 실제 라우트 8개 불변 + mock-shop
결정성(2회 해시 동일).

### 13.10 같은 결함 클래스가 다른 두 곳에도, 그리고 JSX 프래그먼트 (2026-09-15)

§13.9 직후 한 번에 3건이 왔다 — 이번엔 라우트 스캐너가 아니라 **같은 결함
패턴이 반복되는 다른 코드 경로**들이었다.

- **P1 — `main()`의 단일 파일 분기가 크기 상한을 완전히 우회**: 대상 인자로
  디렉터리가 아니라 파일 하나를 직접 지정하면 `root.is_file()` 분기를 타
  `read_sources()`를 아예 거치지 않아, 그 안의 `MAX_FILE_BYTES` 상한이 전혀
  적용되지 않았다 — 거대한 적대적 파일을 직접 지정하면 상한 없이 모든
  추출기에 그대로 들어갔다. `read_sources()`와 동일한 크기 검사를 이 분기에도
  추가.
- **P1 — `extract_api_calls`의 `fetch(...)` 스캔에 §13.4와 같은 이차식 결함**:
  각 `fetch("x"` 발견마다 독립적으로 `_find_matching_paren`을 호출해 닫는
  `)`를 찾는 구조가, 라우트 추출기가 §13.4에서 겪은 것과 정확히 같은 근본
  원인이었다 — 닫히지 않는 `fetch("x" ` 접두어 반복에 파일 크기 상한 훨씬
  안에서도 이차식(20KB만으로 1초+)으로 느려짐을 재현. §13.5와 같은 단일
  좌→우 패스로 교체(`_iter_fetch_calls`) — 20KB 0.002초, ~2MB 0.18초로 확인.
- **P2 — JSX 프래그먼트 단축 문법 `<>` 미인식**: 여는 프래그먼트 `<>`는 다음
  문자가 식별자도 `/`도 아니라(바로 `>`) §13.8/13.9의 태그 감지 조건에
  안 걸려, 내용이 이전 맥락(대개 "code")에 그대로 남아 그 안의 아포스트로피를
  문자열 시작으로 오인했다(§13.7과 같은 증상 재발). 프래그먼트는 속성이 없어
  속성 목록 스캔이 불필요하므로, 만나는 즉시 "text"로 전환하도록 전용 분기
  추가(닫는 `</>`는 `/`로 시작해 기존 일반 태그 감지로 이미 정상 처리됨).

재검증: 세 재현 케이스 모두 해결 + 정상 `fetch`/`axios` 호출 추출 결과 불변 +
§13.6~13.9 전체 라우트 재현 케이스 재통과 + mock-shop 실제 라우트 8개·API
호출 결과 불변 + mock-shop 결정성(2회 해시 동일).

### 13.11 §13.10 자체의 잔존 2건 — fetch 문자열, TSX 제네릭 화살표 (2026-09-15)

§13.10 직후 그 수정 자체에서 다시 2건.

- **P2 — fetch 인자 문자열 안 짝 안 맞는 괄호로 뒤의 정상 호출까지 누락**:
  §13.10에서 `_iter_fetch_calls`를 단일 패스로 바꾸며 "닫는 `)`를 못 찾으면
  전체 탐색 종료" 규칙을 그대로 가져왔는데, fetch는 라우트와 달리 인자가
  JSON 바디 등 **문자열 값 안에 짝 안 맞는 괄호가 실제로 흔히 나올 수 있는**
  영역이다(예: `fetch("/one", {body: "("})`) — 순수 괄호 카운팅이 이를
  구조적 괄호로 오세어 EOF까지 스캔, 그 뒤의 멀쩡한 `fetch("/two")`까지
  통째로 누락됨을 재현. 이전의 독립 정규식 반복 방식은 이런 경우에도 한
  매치만 이상해질 뿐 나머지는 계속 찾았는데, 성능 최적화 과정에서 이 견고함을
  실수로 잃었던 것 — `_find_matching_paren_skip_strings`(문자열/템플릿
  리터럴 안 괄호는 깊이에서 제외, 라우트 스캐너처럼 JSX 텍스트 구분이
  필요 없어 더 단순한 버전으로 충분)로 교체해 해결.
- **P2 — TSX 제네릭 화살표 함수 `<T,>(x: T) => x` 오인**: `.tsx`에서 JSX와
  제네릭을 구분하려고 일부러 트레일링 콤마를 붙이는 관용구(`<T,>`)가, §13.9의
  비교연산자 판정(직전 문자 기준)으로는 걸러지지 않아(직전이 `{`라 "태그
  시작"으로 판정) 태그로 오인되고 그 `>`가 태그 종료로 소비되면서 뒤따르는
  화살표 함수 본문이 JSX 텍스트로 잘못 처리됐다 — `<`의 모호성이 비교연산자
  뿐 아니라 제네릭에도 있었던 것. JSX 태그 이름 뒤에는 콤마가 올 수 없다는
  점(문법 오류가 됨)에 착안해, 식별자 뒤 첫 비식별자 문자가 콤마면 태그가
  아니라 제네릭 매개변수 목록으로 판정하는 `_looks_like_generic_params`를
  추가.

재검증: 두 재현 케이스 + 다중 제네릭(`<T, U>`) 케이스 + §13.6~13.10 전체
재현 케이스(라우트/fetch 모두) 재통과 + ReDoS 재현 케이스 여전히 빠름(라우트
~0.36초, fetch ~0.28초, 각 ~2MB) + mock-shop 실제 라우트 8개·API 호출 결과
불변 + mock-shop 결정성(2회 해시 동일).

### 13.12 제약 제네릭 + 중첩 fetch — fetch 스캐너 설계를 아예 바꿈 (2026-09-15)

§13.11 직후 2건 더 — 하나는 §13.11의 제네릭 판정을 좁게 걸었던 것, 하나는
fetch 스캐너 설계 자체의 근본 빈틈이었다.

- **P2 — 제약 제네릭 `<T extends object>` 미인식**: §13.11의
  `_looks_like_generic_params`는 식별자 바로 뒤가 콤마인 경우만 봤는데,
  콤마 없이 `extends` 키워드로 제약을 거는 제네릭(`<T extends object>(x: T) => x`)
  은 걸러지지 않아 여전히 태그로 오인됐다 — 식별자 뒤(공백 허용) `extends`
  키워드(단어 경계 확인)가 오는 경우도 함께 판정하도록 확장.
- **P2 — `fetch(...)` 인자 **안에** 중첩된 fetch 호출을 통째로 못 찾음**:
  `fetch("/outer", {headers: await fetch("/token")})`처럼 바깥 호출의 인자
  안에 또 다른 fetch가 있으면, §13.11까지의 "바깥 호출의 닫는 `)` 다음으로
  건너뛰는" 단일 패스 방식은 그 범위 안을 아예 다시 보지 않아 중첩 호출이
  통째로 누락됐다(이전의 독립 정규식 반복 방식은 중첩도 찾았음). 단순히
  "바깥을 찾은 뒤 그 인자 범위 안을 다시 검색"하도록 되돌리면, `fetch(fetch(
  fetch(...)))`처럼 깊이 중첩된 입력에서 바깥쪽일수록 매번 남은 전체를 다시
  훑어 이번엔 **중첩 깊이 기준**으로 또 이차식이 될 위험이 있었다(재현 전
  자체 분석으로 미리 확인).

  설계를 바꿨다: `fetch(` 발견 지점을 먼저 `finditer` 한 번으로 전부 모아두고,
  파일을 정말 한 번만 좌→우로 훑으며 괄호 깊이를 추적 — 발견 지점의 `(`을
  지날 때 (그 순간의 깊이, endpoint)를 스택에 쌓고, 어떤 `)`에서 깊이가
  스택 맨 위가 열렸던 깊이로 돌아오면 그 호출을 완결로 보아 즉시 내보낸다.
  각 문자를 정확히 한 번씩만 방문하므로 중첩 개수·깊이와 무관하게 전체
  비용이 파일 길이에 선형이고, 부수 효과로 §13.11의 "호출 하나가 이상하면
  나머지 다 못 찾음" 취약점도 함께 해소됐다(이제 한 호출이 안 닫혀도
  스택에 남을 뿐, 다른 호출 탐색을 막지 않는다). 이제 이 설계로 불필요해진
  `_find_matching_paren_skip_strings`는 제거.

재검증: 두 재현 케이스 + 제약·다중 제네릭 조합 케이스 + 중첩 fetch 3단계
전부 추출 + 깊은 중첩 스트레스(500단계, 0.0005초) + 적대적 대량 반복
스트레스(~600KB, 0.17초) + 기존 평탄 ReDoS 케이스(~2MB, 0.57초) +
§13.6~13.11 전체 재현 케이스(라우트/fetch) 재통과 + 정상 fetch/axios 호출
추출 불변 + mock-shop 실제 라우트 8개·API 호출 결과 불변 + mock-shop
결정성(2회 해시 동일).

### 13.13 시크릿 스캔의 별도 상한 우회, 블록 주석, 템플릿 보간 (2026-09-15)

§13.12 직후 3건 — 이번엔 세 번째로 다른 코드 경로(`extract_secret_findings`)
에서 §13.10과 같은 크기 상한 우회 패턴이 또 나왔고, 나머지 둘은 라우트/fetch
스캐너 각각의 잔여 빈틈이었다.

- **P1 — `extract_secret_findings`가 파일을 독립적으로 다시 읽어 크기 상한을
  우회**: `main()`의 단일 파일 분기(§13.10)와 같은 패턴이 세 번째로 발견된
  곳이다 — 시크릿 스캔은 `read_sources()`가 이미 건너뛴 거대 파일을 **자기
  루프 안에서 다시 직접 `read_text()`** 해, 정상 파일 하나와 거대 파일 하나가
  같이 있는 디렉터리를 대상으로 주면 거대 파일이 상한 없이 모든 시크릿
  정규식을 그대로 통과했다. `read_sources()`와 동일한 크기 검사를 이 루프
  에도 추가(건너뛰면 stderr 경고도 일관되게 출력).
- **P2 — 라우트 스캐너: 블록 주석 안 아포스트로피**: `<Page /> /* don't */`
  처럼 코드 맥락의 `/* ... */` 블록 주석 안에 아포스트로피가 있으면, 주석을
  인식하지 못해 그 따옴표를 문자열 시작으로 오인해 §13.7과 같은 증상(스캔이
  EOF까지 튀어 라우트 전체 소실)이 재발했다 — `_blank_full_line_comments`는
  줄 전체 `//` 주석만 지우고 블록 주석은 애초에 처리 대상이 아니었다
  (reference.md 1-I에 명시). 코드 맥락에서 `/*`를 보면 대응하는 `*/`까지
  (없으면 파일 끝까지) 건너뛰도록 추가.
- **P2 — fetch 스캐너: 템플릿 리터럴 보간(`${...}`) 안의 fetch 미탐지**:
  템플릿 리터럴을 백틱째로 통째 건너뛰는 방식이라 `` `resp: ${fetch("/inner")}` ``
  처럼 보간 **안에** 있는 fetch 호출은 아예 안 보였다(이전의 독립 정규식
  방식은 보간 안까지 찾았음). `$`+`{`를 만나면 대응하는 `}`까지는 다시 code
  맥락(괄호/따옴표 정상 추적)으로 들어가도록 고쳤다 — `{`/`}`는 라우트
  스캐너와 같은 스택 방식(여는 시점 mode를 저장했다가 닫는 시점에 복원)으로
  다뤄, 보간 안의 일반 객체 리터럴 `{}`과도 올바르게 구분된다.

재검증: 세 재현 케이스 모두 해결 + 안 닫히는 블록 주석이 안전하게 처리되는지
(해시 안 걸림) + 중첩 템플릿·보간 안 객체 리터럴·일반 텍스트가 섞인 복합
fetch 케이스 + §13.6~13.12 전체 재현 케이스(라우트/fetch) 재통과 + ReDoS
케이스 여전히 빠름(라우트 ~0.45초, fetch ~0.61초, 각 ~2MB) + mock-shop 실제
라우트 8개·API 호출 결과 불변 + mock-shop 결정성(2회 해시 동일).

### 13.14 fetch에도 주석, 제네릭 기본값, 그리고 세 번째 스캐너의 이차식 결함 (2026-09-15)

§13.13 직후 4건 — 이번엔 세 번째 정규식 스캐너(`_find_if_return_pairs`, 규칙/
메시지 추출용)에서도 같은 이차식 결함 클래스가 나왔고, 나머지는 §13.13의
주석 처리·§13.12의 제네릭 판정이 fetch/라우트 각각에 덜 미친 부분이었다.

- **P2 — fetch 스캐너에도 블록 주석 처리 누락**: §13.13에서 라우트 스캐너
  에만 `/* ... */` 처리를 추가하고 fetch 스캐너는 빠뜨렸다 —
  `const x = 1; /* don't */ fetch("/real")`에서 주석 안 아포스트로피가
  문자열 시작으로 오인돼 `/real` 호출이 통째로 누락됨을 재현. 라우트
  스캐너와 동일한 방식(코드 맥락에서 `/*`를 보면 대응 `*/`까지 건너뜀)을
  fetch 스캐너에도 추가.
- **P2 — 기본값이 있는 제네릭 `<T = unknown>` 미인식**: §13.12는 콤마와
  `extends`만 봤는데, 콤마도 `extends`도 없이 `=`로 기본값을 지정하는
  제네릭(`<T = unknown>(x: T) => x`)은 여전히 태그로 오인됐다 — 식별자 뒤
  (공백 허용) `=`가 오되 그 다음 문자가 `=`나 `>`가 아닌 경우(즉 `==`/`=>`가
  아닌 단독 `=`)도 제네릭으로 판정하도록 확장.
- **P1 — `_find_if_return_pairs`의 이차식 결함**: `extract_rules`/
  `extract_messages`가 공유하는 이 헬퍼도 각 `if (` 발견마다 독립적으로
  `_find_matching_paren`을 불러 닫는 `)`를 찾는 같은 구조였다 — 닫히지 않는
  `if (` 접두어 반복에 32KB만으로 7초 가까이 걸림을 재현(3번째로 발견된
  같은 결함 클래스: extract_routes → extract_api_calls → 이번엔
  extract_rules/extract_messages). §13.12의 fetch 스캐너와 같은 설계(발견
  지점을 먼저 모아두고 파일을 한 번만 훑으며 괄호 깊이를 스택으로 추적)로
  교체 — `if (cond1) { if (cond2) {...} }`처럼 콜백 등으로 조건 텍스트
  안에 다른 if가 있어도 안쪽 것을 놓치지 않고, 문자열 리터럴 안의 괄호도
  깊이에서 제외한다(이번엔 재발이 뻔히 예상돼 미리 포함). 이제 불필요해진
  `_find_matching_paren`은 제거.

재검증: 세 재현 케이스 모두 해결 + 32KB 입력 6.9초→0.0096초, ~2MB도 1.07초 +
정상 규칙/메시지 추출 결과 불변(콜백 안 중첩 if, 조건 문자열 안 괄호 케이스
포함) + 다중 제네릭 기본값 조합 + §13.6~13.13 전체 재현 케이스(라우트/fetch)
재통과 + mock-shop 실제 라우트 8개·API 호출 결과 불변 + mock-shop 결정성
(2회 해시 동일).

### 13.15 네 번째 이차식 스캐너 + 정규식 리터럴, 그리고 자체 발견한 회귀 2건 (2026-09-15)

§13.14 직후 3건 — 이번엔 네 번째 이차식 결함(`extract_rules`의 min/max
정규식), fetch 스캐너에 빠진 블록 주석, 그리고 **완전히 새로운 모호성 클래스**
(정규식 리터럴)였다. 수정하는 과정에서 자체 테스트로 회귀 2건을 추가로
잡았다.

- **P1 — `extract_rules`의 min/max 속성 스캔, 네 번째 같은 결함 클래스**:
  `min=\{(\w+)\}[^>]*?max=\{...\}`의 무경계 lazy `[^>]*?`가, `>`도 `max=`도
  없이 `min={x}`가 반복되는 입력에서 매 시도마다 나머지 파일 끝까지 훑어
  이차식이었다(112KB만으로 10초 가까이) — extract_routes → extract_api_calls
  → extract_rules(if-return) → 이번이 네 번째. 두 속성은 같은 태그의 인접
  속성이라 실제로는 몇십 자 이내이므로 300자 상한(`[^>]{0,300}?`)으로 교체.
- **P2 — fetch 스캐너에 블록 주석 처리가 또 빠짐**: §13.14에서
  `_find_if_return_pairs`에는 넣고 fetch 스캐너엔 또 빠뜨렸다(§13.13에서
  라우트→fetch로 넘어갈 때와 같은 실수 패턴) — 추가.
- **P2 — 정규식 리터럴 안의 아포스트로피, 완전히 새로운 모호성 클래스**:
  `/don't/.test(value)`처럼 정규식 리터럴 안의 아포스트로피를 문자열
  시작으로 오인했다 — "`/`가 나눗셈 연산자인지 정규식 리터럴 시작인지"도
  `<`의 비교연산자/태그 모호성과 같은 종류의 문법적 모호성(실제 JS
  토크나이저도 이 문제를 겪는다)이라, `_looks_like_operator_lt`와 같은
  해소 규칙을 쓰는 `_looks_like_regex_start`를 새로 만들어 라우트·fetch·
  if-return 세 스캐너 모두에 적용했다(재발이 뻔히 예상돼 세 곳에 한 번에).

  **수정 중 자체 테스트로 잡은 회귀 2건**: (1) `</div>`의 `/`가 방금 code
  맥락으로 전환된 직후 직전 문자가 `<`(식별자/닫는 괄호가 아님)라는 이유로
  정규식 시작으로 오판돼 §13.7이 재발 — 닫는 태그의 `<`와 `/`를 한 번에
  건너뛰도록 수정. (2) `<Page label="{" />`의 자체닫힘 `/>`에서 `/` 직전이
  속성값의 닫는 따옴표였는데 "값이 방금 끝난 자리" 판정 집합에 따옴표가
  없어 역시 정규식 시작으로 오판 — 따옴표로 끝난 뒤엔 실제 JS 문법상으로도
  정규식이 올 수 없으므로(나눗셈/자체닫힘만 가능) 따옴표도 그 집합에 포함.

재검증: 세 재현 케이스 + 자체 발견 회귀 2건에 대한 타겟 테스트 + 나눗셈이
정규식으로 오판되지 않는지(`total / 2`) + 8개 라우트가 지금까지 나온 거의
모든 엣지 케이스(텍스트 안 따옴표·아포스트로피, 리터럴 중괄호, 삼항 분기
문자열, 중첩 태그, 비교연산자 vs 태그, 블록 주석, 정규식 리터럴, 프래그먼트)
를 한 파일에 섞은 복합 케이스 + ReDoS 케이스 전부 여전히 빠름(라우트
~0.46초·fetch ~0.70초·if-return ~1.0초·min/max ~0.75초, 각 ~2MB) + 정상
규칙/메시지 추출 불변 + mock-shop 실제 라우트 8개·API 호출 결과 불변 +
mock-shop 결정성(2회 해시 동일).

### 13.16 다섯 번째 이차식 스캐너 + 줄 주석 + 키워드 뒤 정규식 (2026-09-15)

§13.15 직후 3건 — 다섯 번째로 같은 이차식 결함(`disabled={}`), fetch
스캐너에 줄 주석 처리 누락, 그리고 `_looks_like_regex_start`가 `return`
같은 키워드 뒤를 못 알아채는 한계였다. 이번엔 처음부터 지금까지 나온
모든 교훈(문자열·블록주석·줄주석·정규식 인식)을 한 번에 넣어서 짰다.

- **P1 — `disabled={...}` 스캔, 다섯 번째 같은 결함 클래스**: extract_routes →
  extract_api_calls → extract_rules(if-return) → min/max → 이번엔
  `disabled={}`까지, 매 발견마다 독립적으로 `_find_matching`을 불러 닫는
  `}`를 찾는 같은 구조가 또 있었다(16KB 미만 입력에서 0.77초). 처음부터
  fetch/if-return과 같은 단일 패스 설계로 새로 작성(`_iter_disabled_conditions`)
  하면서, 이번 라운드에서 나온 문자열·블록주석·줄주석·정규식 리터럴 인식을
  전부 포함해 짰다 — 같은 재발을 또 기다리지 않기 위함. 이제 완전히
  불필요해진 `_find_matching`(마지막 남은 호출부였음)은 제거.
- **P2 — fetch 스캐너에 줄 주석(`//`) 처리 누락**: `_blank_full_line_comments`
  는 `//` 앞에 공백이 있을 때만 트레일링 주석으로 지운다(URL 오손상 방지
  목적) — `const x=1;// don't`처럼 공백 없이 바로 붙으면 안 지워지고 남아
  아포스트로피가 문자열 시작으로 오인됐다. 블록 주석과 같은 자리에 줄 주석
  처리를 추가했는데, 이번엔 **라우트·fetch·if-return 세 스캐너 모두에 한
  번에** 적용했다(정규식 리터럴 때와 같은 이유 — 한 곳에서 발견되면 나머지도
  같은 문제를 가질 게 뻔함).
- **P2 — `return`/`throw` 등 키워드 뒤의 정규식 리터럴 오판**: `_looks_like_regex_start`
  는 "`/` 직전이 식별자 문자로 끝나면 나눗셈"으로 판정하는데, `return`도
  식별자 문자("n")로 끝나 `return /don't/.test(value)`의 `/`를 나눗셈으로
  오판했다 — 키워드 뒤는 언제나 새 표현식이 시작하는 자리이지 값이 끝난
  자리가 아니다. 직전 단어를 추출해 `return`/`throw`/`yield`/`case`/
  `typeof`/`instanceof`/`in`/`of`/`new`/`delete`/`void`/`do`/`else` 키워드
  목록에 있으면 식별자로 끝났어도 정규식 시작으로 뒤집도록 수정 — 이
  헬퍼는 세 스캐너가 공유하므로 한 번 고치면 전부 적용된다.

재검증: 세 재현 케이스 + `throw` 키워드 케이스 + 정상 `disabled={}` 추출
결과 불변 + §13.6~13.15 전체 재현 케이스 재통과 + 나눗셈이 여전히 정규식으로
오판되지 않는지 + ReDoS 케이스 전부 여전히 빠름(라우트 ~0.50초·fetch
~0.60초·if-return ~0.98초·min/max ~0.75초·disabled ~0.62초, 각 ~2MB) +
mock-shop 실제 라우트 8개·API 호출·결정성(2회 해시 동일) 불변.

### 13.17 라우트 스캐너의 남은 잔여 한계 — 중첩 템플릿 리터럴 (2026-09-15)

라우트 스캐너는 §13.6부터 계속 "템플릿 리터럴의 `${...}`는 완벽히 다루지
않는다"를 알려진 잔여 한계로 명시해왔는데(fetch 스캐너는 §13.13에서 이미
`${...}` 보간을 코드 맥락으로 추적하도록 고쳤지만 라우트 스캐너는 그대로
뒀었다), 실제로 버그로 확인됐다: `` `x${`don't`}` `` 처럼 템플릿 리터럴
**안에** 또 템플릿 리터럴이 중첩되면, 백틱을 "다음 백틱까지 통째로"
건너뛰는 방식이 안쪽 여는 백틱을 바깥 문자열의 닫는 백틱으로 오인해 너무
일찍 문자열을 끝내고, 그 뒤 "don't"의 아포스트로피를 문자열 시작으로
오인했다(§13.7과 같은 증상 재발).

fetch 스캐너가 §13.13에서 이미 검증한 것과 같은 설계를 그대로 포팅했다:
백틱 문자열 안에서는 "template" 모드로 문자 단위 스캔하다 `${`를 만나면
그 `{`를 라우트 스캐너가 이미 갖고 있던 일반 `{`/`}` 스택(여는 시점 mode를
저장했다가 닫는 시점에 복원하는 `brace_modes`)에 "template"으로 밀어 넣고
"code"로 전환 — 보간 내용은 정상적인 코드 맥락(문자열/중첩 백틱 포함)으로
재귀적으로 다뤄지고, 대응하는 `}`에서 기존 일반 `}` 처리가 자동으로
"template"으로 복귀시킨다. 별도의 새 자료구조 없이 기존 스택을 그대로
재사용한 덕분에 변경 폭이 작았다.

재검증: 재현 케이스 + 보간 안에 객체 리터럴이 있는 경우·평범한 텍스트만
있는 템플릿·보간 없이 리터럴 `{`만 있는 템플릿 + §13.6~13.16 라우트 재현
케이스 전체 재통과 + ReDoS 재현 케이스(라우트 ~0.53초/~2MB) + 안 닫히는
중첩 백틱 대량 반복 스트레스(0.08초) + mock-shop 실제 라우트 8개·결정성
(2회 해시 동일) 불변.
...
### 13.19 if-return/fetch/disabled 세 스캐너 모두에 JSX 텍스트 맥락 추가 (2026-09-15)

§13.18에서 라우트 스캐너의 "`return` 뒤 JSX" 문제를 고치자마자, 같은
근본 원인의 반대쪽 — if-return/fetch/disabled 세 스캐너 모두 **파일
전체를 좌→우로 훑는데 JSX 텍스트 맥락을 전혀 모른다** — 가 세 건 한
번에 보고됐다. 세 스캐너 모두 지금까지 "코드 맥락"만 알았지 "지금
JSX 자식 텍스트 안이라 따옴표가 그냥 글자"라는 개념이 없어서, `if (`/
`fetch(`/`disabled={`보다 **앞에** 나오는 평범한 마크업 텍스트 안의
아포스트로피(`<p>Don't stop</p>`)를 문자열 시작으로 오인 — 파일 끝까지
"문자열 안"으로 착각해 그 뒤의 정상 발견을 전부 놓쳤다.

세 스캐너 모두 라우트 스캐너 `_find_matching_skip_strings`의 태그
감지·모드 전환 로직(`mode`="code"/"text", `<Tag>`/`</Tag>`로 전환,
`_looks_like_operator_lt`/`_looks_like_generic_params`로 비교연산자·
제네릭과 구분, `text_stack`으로 복귀)을 그대로 포팅했다. 다만 라우트
스캐너는 특정 `{`(`element={`)부터 시작해 그 짝 `}`까지만 찾으면
끝나는 반면, 이 세 스캐너는 파일 전체를 훑으며 **다른 목적의 깊이**를
이미 추적하고 있어(if-return/fetch: `(` 깊이, disabled: `{` 깊이) 태그
자신의 종료 `>`를 찾는 목표 깊이를 무엇으로 잡을지가 스캐너마다
달랐다:
- if-return/fetch: 이미 있던 `brace_modes`(템플릿 보간 `${}` 모드
  복원용) 스택의 길이를 그대로 재사용 — 템플릿 보간이든 JSX 표현식이든
  `{`/`}`는 똑같이 그 스택에 쌓이므로 자연스럽게 맞아떨어졌다.
- disabled: 이 스캐너의 기존 `depth`(모든 `{`/`}`를 세는 카운터, 자신의
  `disabled={` 매칭에도 쓰임)를 그대로 재사용 — 같은 문자들을 세는
  카운터이므로 JSX 표현식 깊이와 항상 일치한다.

부작용 하나를 자체 발견해 같이 고쳤다: `mode`에 "text"를 추가하면서
if-return/fetch가 이미 갖고 있던 `(`/`)` 처리(각각 if-조건/fetch 인자
깊이 추적용)에 `mode == "code"` 가드가 없었다는 걸 깨달았다 — JSX
텍스트 안의 리터럴 `(`/`)`(예: `<p>(옵션)</p>`)가 그대로 깊이에
섞여 들어가 조건/호출 매칭이 어긋날 뻔했다. 두 스캐너 모두 `(`/`)`
처리에 `mode == "code"` 조건을 추가해 사전 방지했다.

fetch 스캐너에서 재현 케이스로 준 `<script>fetch("/real")</script>`를
그대로 적용하면 또 다른 문제가 드러났다 — `<script>` 태그의 자식은
일반 JSX 컴포넌트와 달리 텍스트가 아니라 **그대로 JS 코드**인데,
새로 추가한 로직이 모든 여는 태그를 똑같이 "text"로 전환해버리면
`<script>` 안의 `fetch(...)`까지 텍스트로 지나쳐 다시 놓쳤다(자체
재검토로 발견, 커밋 전에 재현·수정). 태그 이름을 같이 기록해두고
`<script>`(대소문자 무관)만 "text"가 아니라 "code"를 유지하도록
예외를 뒀다.

재검증: 세 재현 케이스 모두 수정 전 실패 → 수정 후 통과. `<script>`
자체 발견 회귀도 수정 후 통과 확인. 회귀 방지로 §13.10~13.18의 기존
재현 케이스 전체(중첩 fetch, 블록 주석 앞 if, disabled 중첩 중괄호/
화살표 등) 재통과. ReDoS 재현(안 닫히는 태그/disabled/if 접두어 대량
반복) 세 스캐너 모두 수 ms~수십 ms 수준으로 선형 유지 확인. mock-shop
라우트 8개·API 호출 2개·규칙 6개·2회 해시 결정성 불변.

---

### 13.20 실행 가능한 핸들러 속성값, 그리고 if-return/disabled의 `<script>` 누락 (2026-09-15)

§13.19를 푸시하자마자 두 건이 더 보고됐다 — 하나는 fetch 스캐너의 새로운
갭, 하나는 §13.19에서 fetch 스캐너에만 넣고 if-return/disabled 스캐너에는
빠뜨렸던 `<script>` 예외의 재발이었다(자업자득: 같은 문제를 세 스캐너
중 하나에서만 고치고 나머지에 안 옮긴 결과).

1. **실행 가능한 핸들러 속성값(fetch 스캐너)**: `<button @click="fetch('/api')">`
   /`<button onclick="fetch('/api')">`처럼 Vue 디렉티브·HTML 인라인
   핸들러의 속성값은 문자열이 아니라 그대로 실행되는 코드인데, 이 스캐너는
   모든 속성값 따옴표를 무조건 불투명한 문자열로 건너뛰어 그 안의 fetch
   호출을 놓쳤다. 새 헬퍼 `_looks_like_executable_attr`로 (직전이 `=`이고
   그 앞 속성 이름이 `on행동`/`@행동`/`v-on:행동` 패턴인지만 보는 정적
   판정) 핸들러 속성인지 가린 뒤, 그런 속성값은 건너뛰지 않고 code 맥락
   그대로 둔 채(여는 따옴표 문자만 `attr_quote_stack`에 기억해두고 같은
   문자를 다시 만나면 닫힘으로 판정) 안의 `fetch(...)`가 정상적으로 괄호
   깊이 추적에 걸리게 했다. 속성값 안에 다른 종류의 따옴표로 중첩된
   일반 문자열(`fetch('/api')`의 `'/api'`)은 기존 문자열-건너뛰기 경로가
   그대로 처리해 추가 파싱 없이도 올바르게 작동한다.
2. **if-return/disabled 스캐너의 `<script>` 누락**: §13.19에서 이미 다룬
   문제(`<script>` 자식은 JSX 텍스트가 아니라 JS 코드)를 fetch 스캐너에만
   고치고 if-return/disabled 스캐너에는 옮기지 않았다 — `<script
   setup>if (x) return "bad";</script>` 같은 Vue SFC 패턴이 그대로
   재발했다. 두 스캐너 모두에 태그 이름 기록 + `<script>`(대소문자 무관)
   예외를 포팅했다(disabled 스캐너 쪽은 아직 보고되지 않았지만 같은 근본
   원인이 그대로 남아있어 함께 고쳤다).

재검증: 세 재현 케이스(핸들러 속성 fetch 2종 + `<script setup>` if-return)
모두 수정 전 실패 → 수정 후 통과. 회귀 방지로 일반(비핸들러) 속성값
안의 `fetch(...)`류 텍스트가 여전히 코드로 스캔되지 않는지(오탐 방지),
`<script>` 안의 `disabled={}` 인식, §13.10~13.19 기존 재현 케이스 전체
재통과 확인. ReDoS 재현(안 닫히는 핸들러 속성 대량 반복) 선형 유지
확인. mock-shop 라우트 8개·API 호출 2개·규칙 6개·2회 해시 결정성 불변.
