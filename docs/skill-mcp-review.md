...
### 13.8 예고했던 바로 그 갭 — 좁은 휴리스틱을 작은 상태기계로 교체 (2026-09-15)

§13.7 답글에서 "`=` 없이 오는 문자열 리터럴(예: 삼항 분기)까진 못 다룬다"고
미리 밝혔던 그 한계를, 리뷰봇이 정확히 그 자리에서 찾아냈다:
`<Page label={condition ? "{" : "x"} />` — `"{"`가 `=` 뒤가 아니라 `?` 뒤에
오는 삼항 분기라 §13.7의 "`=` 뒤 따옴표만" 규칙을 통과하지 못했고, 안의
리터럴 `{`가 다시 깊이를 흐트러뜨려 파일의 라우트가 전부 사라졌다(§13.6/13.7과
같은 증상 3연속).

세 라운드(§13.6 속성값 안 리터럴 중괄호 · §13.7 텍스트 안 아포스트로피 ·
§13.8 `=` 없는 표현식 안 문자열)를 관통하는 진짜 원인은 "따옴표 위치"가 아니라
**"지금이 JSX 텍스트 맥락인가, 코드(태그 속성 목록/표현식) 맥락인가"**였다.
더 좁은 휴리스틱을 또 추가하는 대신, `_find_matching_skip_strings`를 이 맥락을
실제로 추적하는 작은 상태기계로 다시 썼다:

- `mode`("code"/"text")로 지금 따옴표가 문자열 구분자인지(코드) 그냥 글자인지
  (텍스트) 판정.
- `{`/`}`는 맥락과 무관하게 항상 깊이로 세고(JSX 규격상 이스케이프 없는 `{`는
  텍스트에서도 항상 표현식 시작), 여는 시점의 mode를 스택에 저장했다가 닫는
  시점에 복원 — 목표 깊이(0) 판정은 이 스택 그대로 유지.
- `<태그`/`</태그`를 만나면 그 태그 자신의 종료 `>`를 찾을 때까지 mode를
  "code"로 전환해 속성 목록을 정상 처리하고, 종료 `>`에서 "닫는 태그면 부모
  텍스트로 복귀 / 자체 닫힘이면 시작 전 mode로 복귀 / 여는 태그(자식 있음)면
  "text"로 전환"을 정확히 구분.

여전히 문자 스캔 기반 근사치(정규식 리터럴, JS 비교연산자 오탐 가능성 등은
미해결 — reference.md 1-I와 같은 성격)지만, 단일 좌→우 패스라는 성질은
유지된다.

재검증: §13.6·13.7·13.8 세 재현 케이스 각각 + **한 파일에 다 섞인 복합
케이스**(텍스트 안 따옴표/아포스트로피, 중첩 태그, `<`/`>`처럼 보이는 텍스트,
이스케이프된 따옴표, 표현식 안 삼항 분기 문자열까지 5개 라우트) 전부 정상
추출 + ReDoS 재현 케이스 여전히 빠름(2MB 근처 ~0.32초) + 정상 다중 라우트
추출 불변 + mock-shop 실제 라우트 8개·값 불변 + mock-shop 결정성(2회 해시
동일).
