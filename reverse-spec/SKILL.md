...
항상 적용한다. 정책 규칙화(Step 2)는 사실 표의 조건·상수·규칙 행을 근거로 수행한다.
...
2. 모듈마다 extract.py를 **모듈별 출력 경로로** 실행한다(이미 사전 승인된 명령):
   ...
   ```bash
   python ${CLAUDE_SKILL_DIR}/scripts/extract.py src/pages --output reverse-spec-output/_facts_pages.md
   python ${CLAUDE_SKILL_DIR}/scripts/extract.py src/admin --output reverse-spec-output/_facts_admin.md
   ```
...
2. Scope & Product Policy
   2.1 흐름 (흐름도)
       - 전체 화면 흐름 SVG 다이어그램 (scripts/flowgen.py로 생성, 아래 Step 4-B)
...
- 코드 근거가 없는 전환은 라벨에 `[추정]`을 붙이고 `dashed: true`로 표시한다.
...
```bash
python ${CLAUDE_SKILL_DIR}/scripts/render.py --input reverse-spec-output/_spec_body.md --title "역기획 정책서" --accent "#2c2c2c" --outdir reverse-spec-output --name reverse_spec
```
...
- 출력 파일: `reverse-spec-output/reverse_spec_YYYYMMDD_HHMMSS.pdf` + 동일 이름 `.html`
